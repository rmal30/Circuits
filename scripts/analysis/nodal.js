...
function nodalAnalysis(circuit) {
    ...
    for (const edgeID in graph.edges) {
        if (["vdc", "vac", "vs-"].some((prefix) => edgeID.includes(prefix))) {
    ...
    for (let c = 0; c < (1 << diodes.length); c++) {
        ...
        const voltageComponents = getComponents(circuit.components, ["vdc", "vac"]);
        ...
        const voltageAmpComponents = getComponents(circuit.components, ["vcvs"]);
        ...
        voltages.push(ComplexMatrix.scalarMultiply(ComplexMatrix.multiply(voltMatrix, ComplexMatrix.transpose([nodeVoltages])), -1));
    }
